...
SELECT @emailLogin = email from inserted
 ...
 DECLARE @passwordLogin BINARY(64) 
 SELECT @passwordLogin = password from inserted


 DECLARE @passwordByEmail BINARY(64) 
...
CREATE PROCEDURE loginProcedure
    ...
    @password /*parameter name*/ varchar(60) /*datatype_for_param1*/  /*default_value_for_param2*/
...
VALUES
( -- second row: values for the columns in the list above
  @email,  HASHBYTES('SHA2_512', @password)
)
    GO
